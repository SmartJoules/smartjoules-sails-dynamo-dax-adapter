...
A Waterline adapter for accessing DynamoDB with SailsJS v1+, with optional Amazon DAX support.
...
```sh
$ npm install sails-dynamo-v1-dax
```
...
## DAX Configuration (Optional)

If you provide a DAX endpoint, query/read/write requests use DAX via `DocumentClient`.
Table bootstrap operations (e.g. `listTables`, `createTable`) still use direct DynamoDB.

Supported config keys:
- `daxEndpoints` (recommended, array or comma-separated string)
- `daxEndpoint`
- `daxClusterEndpoint`
- `daxUrl`
...
module.exports.datastores = {
  default: {
    ...
    daxEndpoints: [
      "my-cluster.abcde.clustercfg.dax.use1.cache.amazonaws.com:8111"
    ]
  }
};
```
...
- atif-salafi (atif8486@gmail.com)

## License

This `sails-dynamo-v1-dax` adapter is available under the **MIT license**.
