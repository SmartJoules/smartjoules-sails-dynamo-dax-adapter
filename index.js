...
function normalizeDaxEndpoints(datastoreConfig) {
  const rawEndpoints =
    datastoreConfig.daxEndpoints ||
    datastoreConfig.daxEndpoint ||
    datastoreConfig.daxClusterEndpoint ||
    datastoreConfig.daxUrl;
  ...
  if (Array.isArray(rawEndpoints)) {
    return rawEndpoints.map((endpoint) => String(endpoint).trim()).filter(Boolean);
  }

  return String(rawEndpoints)
    .split(',')
...
module.exports = {
  ...
  identity: 'sails-dynamodb-v1-dax',
  ...
  updateCredentials: async function (datastoreConfig) {
    ...
    // Use DAX for request path only when an endpoint is configured.
    // Keep base DynamoDB client for admin/bootstrap actions (list/create table).
    if (daxEndpoints.length > 0) {
      try {
        const dax = new AmazonDaxClient({
          endpoints: daxEndpoints,
          region,
          credentials
        });
        ...
        console.warn(
          `[sails-dynamo-v1-dax] Failed to initialize DAX client, falling back to DynamoDB DocumentClient: ${error.message}`
        );
      }
    }
